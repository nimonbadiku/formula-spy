...
const terms = [
  ...
  'Tetramethyl Acetyl','Menthol','Benzyl Alcohol',
  'Cocamide','Laminaria','Dimethiconol Hyaluronate'
];
...
terms.forEach(t => {
  ...
  if (f.length) {
    f.forEach(i => {
      const shamScore = i.product_roles && i.product_roles.shampoo ? i.product_roles.shampoo.score : 'NO_ROLE';
      console.log(`FOUND: "${i.name}" | cat: ${i.category} | tags: ${JSON.stringify(i.tags)} | ionic: ${i.ionic_charge} | shampoo_score: ${shamScore}`);
    });
  } else {
