# Proposed Scoring Fixes — Iteration 8

Generated from 5 diagnosed error pattern(s).
Avg absolute error entering this iteration: **17.94**

## protein_overrating (proteinBalance.ts)
**Severity:** high | **Affected cases:** 3 | **Avg error:** +16.21

**Diagnosis:** 3 protein-containing products scored 16.2 points above expected on average. Protein bonus modifiers (DAMAGED_PROTEIN_BONUS, PROTEIN_HUMECTANT_SYNERGY_BONUS) may be too generous or the stacking penalty (PROTEIN_STACK_PENALTY) too weak.
...
**Severity:** high | **Affected cases:** 5 | **Avg error:** -25.95

**Diagnosis:** 5 shampoos with harsh surfactants scored 25.9 points below expected. STRONG_DRY_SCALP_PENALTY (0.78) or STRONG_DAMAGED_PENALTY (0.82) may be too harsh, reducing even oily-hair profiles unnecessarily.
...
**Severity:** high | **Affected cases:** 2 | **Avg error:** +23.08

**Diagnosis:** 2 silicone-heavy products scored 23.1 points above expected. Buildup risk penalties for heavy silicones may be insufficient for curly/coily profiles.
...
**Severity:** high | **Affected cases:** 7 | **Avg error:** -25.83

**Diagnosis:** 7 low-porosity profiles with silicones scored 25.8 points below expected. Low-porosity penalty for silicones may be stacking excessively (molecular weight + buildup + advanced profile = triple penalty).
...
**Severity:** low | **Affected cases:** 15 | **Avg error:** -28.53

**Diagnosis:** 15 sensitive-scalp profiles underrated by 28.5 pts. Scalp sensitivity may be stacking with cleanser harshness penalties.
